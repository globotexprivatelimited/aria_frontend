...
export default function RoomSetup({ onSave, existingCount }: { onSave: (floors: FloorRow[]) => Promise<void>; existingCount: number }) {
  const [floors, setFloors] = useState<FloorRow[]>([{ floor: 1, count: 10, type: "Standard", prefix: "1" }]);
  ...
  return (
    <div style={card}>
      <div style={{ marginBottom: 6 }}>
        <h2 style={{ fontFamily: "Georgia, serif", fontSize: 22, fontWeight: 700, color: INK, margin: 0 }}>Set up your rooms</h2>
        <p style={{ fontSize: 13, color: "#8A8577", marginTop: 4 }}>
          {existingCount > 0 ? "You have " + existingCount + " rooms. Adding more floors below will add to them." : "Tell us how many floors your hotel has, and how many rooms are on each floor. Only you know your property."}
        </p>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 22, paddingTop: 18, borderTop: "1px solid #F0F0EA" }}>
        <div style={{ fontSize: 14, color: INK }}>
          Total: <b style={{ fontFamily: "Georgia, serif", fontSize: 20, color: GREEN }}>{totalRooms}</b> rooms across <b>{floors.length}</b> floor{floors.length === 1 ? "" : "s"}
        </div>
